...
flowchart LR
    ...
    H[历史事件与运维知识] --> X
    G[Git CI/CD 配置变更] --> X
    X --> A[Agent Runtime / Workflow]
    A --> M[模型网关]
    A --> MC[MCP Client]
    MC --> MS[本地 MCP Server]
    MS --> R[指标/日志/Trace/K8s/事件适配器]
    A --> O[分析报告与处置建议]
    O --> N[飞书/企微/邮件/内网通知]
    O --> W[审批与执行网关]
    W --> D[受控运维动作]
...
| Agent Runtime | 执行可恢复 Workflow、限制循环和工具预算、处理中断与人工审批；Mastra 为推荐候选 |
| MCP 工具边界 | 向 Agent 暴露结构化、受限和可审计的诊断工具；遥测采集不经过 MCP |
...
- 第一阶段优先采用确定性 Workflow 编排诊断步骤，只把需要推理的步骤交给 Agent。
- Mastra 是可替换的运行时实现，不是事件中心、权限系统、知识库或执行网关的替代品。
